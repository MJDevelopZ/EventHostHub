...
Version:      1.1.1
Author:       Themify
Description:  Simple contact form. It requires to use with a Themify theme (framework 2.0.6+) or the Builder plugin (v 1.2.5).
	...
	function contact_send() {
		if( isset( $_POST ) && ! empty( $_POST ) ) {
			...
			if( isset( $_POST['contact-recaptcha'] ) ) {
				...
				if( isset( $response['body'] ) ) {
					...
					if( ! true == $response["success"] ) {
						echo '<p class="ui red contact-error">' . __( "Bots are not allowed to submit.", 'builder-contact' ) . '</p>';
						die;
					}
				} else {
					echo '<p class="ui red contact-error">' . __( "Trouble verifying captcha. Please try again.", 'builder-contact' ) . '</p>';
					die;
				}
			}
			...
			if( '' == $name || '' == $email || '' == $message ) {
				$result = '<p class="ui red contact-error">'. __( 'Please fill in the required data.', 'builder-contact' ) .'</p>';
			} else {
				if( ! is_email( $email ) ) {
					$result = '<p class="ui red contact-error">'. __( 'Invalid Email address!', 'builder-contact' ) .'</p>';
				} else {
					...
					foreach( $recipients as $recipient ) {
						...
						if( wp_mail( $recipient, $subject, $message, $headers ) ) {
							$result = '<p class="ui light-green contact-success">'. __( 'Message sent. Thank you.', 'builder-contact' ) .'</p>';
						} else {
							...
							$result = '<p class="ui red contact-error">'. __( 'There was an error. Please try again.', 'builder-contact' ) . '<!-- ' . implode( ', ', $ts_mail_errors ) . ' -->' .'</p>';
						}
					}
