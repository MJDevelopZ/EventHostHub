 ...
 * Description: Add the search your Business Directory plugin listings by ZIP or Postal Code within a given radius.  Requires BD 3.1 or higher.
    ...
    public function widget($args, $instance) {
        ...
        if ( $zip_field_id = wpbdp_get_option( 'zipcode-field' ) ) {
            if ( $field = wpbdp_get_form_field( $zip_field_id ) ) {
                ...
                echo '<label><input type="radio" name="_x[zs_mode]" value="zip" checked="checked" onchange="if (this.checked) { jQuery(\'.zipcode-search-distance-fields\', jQuery(this).parents(\'form\') ).hide(); } " /> ' . sprintf( _x(' Only this %s', 'settings', 'wpbdp-zipcodesearch' ), esc_html( $label ) ) . '</label>';
                echo '<br />';
                echo '<label><input type="radio" name="_x[zs_mode]" value="distance" onchange="if (this.checked) { jQuery(\'.zipcode-search-distance-fields\', jQuery(this).parents(\'form\')).show(); } " /> ' . _x( 'Distance Search', 'settings', 'wpbdp-zipcodesearch' ) . '</label>';
                echo '<br/>';
...
class WPBDP_ZIPCodeSearchModule {
    ...
    public function init() {
        ...
        add_action( 'WPBDP_Form_Field::get_behavior_flags', array( &$this, 'add_field_flags' ), 10, 2 );
        add_action( 'WPBDP_Form_Field::build_quick_search_query', array( &$this, 'build_quick_search_query' ), 10, 6 );
    }
    ...
    public function _register_settings( &$settingsapi ) {
        ...
        $settingsapi->add_setting( $s,
                                   ...
                                   true );
        $settingsapi->add_setting( $s,
                                   'zipcode-quick-search-radius',
                                   _x( 'Default radius to use for Quick Search?', 'settings', 'wpbdp-zipcodesearch' ),
                                   'text',
                                   '5',
                                   _x( 'In the same units specified above.', 'settings', 'wpbdp-zipcodesearch' ) );
    ...
    function add_field_flags( $flags, $field ) {
        $zip_field_id = intval( wpbdp_get_option( 'zipcode-field' ), 0 );

        if ( ! $zip_field_id || $zip_field_id != $field->get_id() )
            return $flags;

        $flags[] = 'quick-search-external';
        return $flags;
    }

    function build_quick_search_query( $field, $q, &$pieces, $search_term, $w_no, &$optimization ) {
        global $wpdb;
        ...
        // Try this as a ZIP search.
        $zip = $this->get_zipcode( $q );
        if ( ! $zip )
            return;

        $radius = max( 0.0, floatval( wpbdp_get_option( 'zipcode-quick-search-radius' ) ) );
        $units = wpbdp_get_option( 'zipcode-units' );

        if ( 'kilometers' == $units )
            $radius = $radius * 0.621371192;

        $posts = $this->find_listings( array( 'center' => $zip->zip, 'radius' => $radius, 'fields' => 'ids' ) );

        if ( ! $posts )
            $pieces['where'] .= ' ) AND 1=0 ( ';
        else
            $pieces['where'] .= sprintf( " OR {$wpdb->posts}.ID IN (%s) ", implode( ',', $posts ) );
    }
    ...
    public function get_supported_databases() {
        ...
        $databases['nz'] = array( _x( 'New Zealand', 'databases', 'wpbdp-zipcodesearch' ), '20150820' );
        ...
        $databases['cz'] = array( _x( 'Czech Republic', 'databases', 'wpbdp-zipcodesearch' ), '20151125' );
