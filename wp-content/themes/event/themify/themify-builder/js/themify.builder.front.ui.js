...
(function ($, _, window, document, undefined) {
    ...
    ThemifyPageBuilder = {
        ...
        applyAll_init: function() {
             jQuery('.style_apply_all').each(function() {
                var $val = jQuery(this).val();

                if (jQuery(this).is(':checked')) {
                    jQuery('.style_'+ $val).not(':first').closest('.themify_builder_field').slideUp();
                }
            });

            ThemifyPageBuilder.applyAll_events();
            ThemifyPageBuilder.applyAll_preSelect();
        },
        // "Apply all" // apply all pre select
        applyAll_preSelect: function() {
            jQuery('.style_apply_all').each(function() {
                var $val = jQuery(this).val(),
                    $field = jQuery('.style_'+ $val),
                ...
                if (!jQuery(this).is(':checked')) {
                    $field.closest('.themify_builder_field').find($fieldFilter).each(function() {
                        if (jQuery(this).val() != '') {
                            $preSelect = false;
                            return false;
                        }
        ...
        applyAll_events: function() {
            jQuery('.style_apply_all').on('click', function() {
                var $val = jQuery(this).val(),
                    $field1 = jQuery('.style_'+ $val).filter(':first'),
                    $field2 = jQuery('.style_'+ $val).not(':first');
                
                if (jQuery(this).is(':checked')) {
                    $field2.closest('.themify_builder_field').slideUp(function() {
                        $field1.closest('.themify_builder_field').find('input, select').each(function() {
                            var ev = (jQuery(this).prop('tagName') == 'SELECT') ? 'change' : 'keyup';
                            jQuery(this).trigger(ev);
                        });
                    });
                }
                else {
                    $field2.closest('.themify_builder_field').slideDown();
                }
            });

           jQuery('.style_apply_all').each(function() {
                var $val = jQuery(this).val(),
                    $inputs = jQuery('.style_'+ $val).filter(':first').closest('.themify_builder_field').find('input'),
                    $selects = jQuery('.style_'+ $val).filter(':first').closest('.themify_builder_field').find('select'),
                
                $callback = function() {
                    var $rel = jQuery(this).data('rel');

                    if (jQuery('.style_apply_all_'+ $rel).is(':checked')) {
                        var $val = jQuery(this).val(),
                            $select = jQuery(this).is('select');
                        
                        jQuery('.style_'+ $rel).not(':first').closest('.themify_builder_field').each(function() {
                            if ($select) {
                                jQuery(this).find('select option').prop('selected', false);
                                jQuery(this).find('select option[value="'+$val+'"]').prop('selected', true).trigger('change');
                            }
                            else {
                                jQuery(this).find('input[type="text"].tfb_lb_option').val($val).trigger('keyup');
                            }
                        });
                    }
                };
        ...
        optionsModule: function (event, isNewModule) {
            ...
            var callback = function () {
                ...
                if ( isNewModule ) {
                    formEl.setAttribute('data-form-state', 'new');
                } else {
                    formEl.setAttribute('data-form-state', 'edit');
                }
            ...
            ThemifyBuilderCommon.Lightbox.open(ajaxOptions, callback);
        },
        ...
        mediaUploader: function () {
            ...
            $body.on('click', '.themify-builder-delete-thumb', function (e) {
                ...
                $(this).parents('.themify_builder_input').find('.themify-builder-uploader-input').val('').trigger('change');
                e.preventDefault();
            });
