	...
	$(document).ready(function($){
		...
		$wpbody.on('click', '.upgrade-framework, .upgrade-theme', function(e){
			...
			if($(this).hasClass('upgrade-theme')){
				type = 'theme';
				themeversion = $('#themeversiontoreinstall').length == 1 ? $('#themeversiontoreinstall').val() : 'latest';
			} else {
				type = 'framework';
			}
			if ( reply ) {
				if ( $(this).parent().hasClass('login') ) {
					...
					//showAlert();
					window.location = blog_url + "wp-admin/admin.php?page=themify&action=upgrade&type=" + type + "&themeversion=" + themeversion + "&login=false";
				}
			}
		});
